#!/usr/bin/env python3
"""
Generate EAPF 2025 practice questions using Cursor's in-editor model (no API key).

Flow:
  1. python generate_questions_cursor.py prompt [--count N]   → writes a prompt file and instructions
...
Uses only Cursor UI for generation; this script only builds the prompt and merges the result.
"""
import argparse
import json
import os
...
def write_prompt(count, skip_content=False):
    ...
    prompt = """You are helping create practice questions for the ArcGIS Pro Foundation 2025 (EAPF_2025) exam.
...
- "domainId": one of: data-management, mapping-visualization, layouts-sharing, performing-analysis
...
- Distribute by domain: about 32% data-management, 36% mapping-visualization, 12% layouts-sharing, 20% performing-analysis.
...
def _open_prompt_file(prompt_path):
    """Open the prompt file with the default application (editor)."""
...
def wizard(count, open_file, run_build):
    """Run the Cursor workflow: generate prompt, wait for user to save new_questions.json, then merge."""
